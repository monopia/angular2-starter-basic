...
export class HangulService {
  ...
  getLetters (consonant: string, surname: boolean) {
    if (surname) {
      return this.hangulMaster[consonant].surnameLetters;  
    } else {
      return this.hangulMaster[consonant].letters;
    }
  }
  mouths = [
    {
  ...
  init() {
    let keys = Object.keys(this.hangulMaster); // ㄱㄴㄷ
    keys.forEach((key) => {
      let hangul = this.hangulMaster[key];
      hangul.surnameLetters = {};
      let letters = Object.keys(hangul.letters); // 가 나 다
      letters.forEach((letter) => {
        if (hangul.letters[letter].surname) {
          hangul.surnameLetters[letter] = hangul.letters[letter];
        }
      })
    })
  }
